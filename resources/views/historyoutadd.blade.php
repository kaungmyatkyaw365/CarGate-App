                            ...
                            <form role="form" action="{{route('historyoutadd.update',$data->id)}}" method="POST">
                               ...
                               {{ $d->type_id == $c->id ? $c->name : ''}}
                                ...
                                <div class="d-flex justify-content-end ">
                                    <input class="btn  bg-gradient-info w-10 mt-4 " type="submit" value="ပေါင်းထည့်မည်">
                                </div>
                                
                                <div class="d-flex mx-2 my-0 justify-content-between ">
                                    <div class="d-flex">
                                    <h6>ကားဆရာ : {{$data->drivername}}</h6>
                                    <h6 class="mx-3">ကားနံပါတ် :  @foreach($drivers as $c)
                               {{ $data->driver_id == $c->id ? $c->carno : ''}}
                                @endforeach</h6>
                                    </div>
