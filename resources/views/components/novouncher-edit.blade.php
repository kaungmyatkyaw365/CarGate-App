<!-- Modal -->
<div class="modal fade" aria-hidden="true" id="novouncherEdit{{$d->id}}" aria-labelledby="exampleModalToggleLabel" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <form role="form" action="{{route('novouncher.destroy',$d->id)}}" method="POST">
                    @csrf
                    @method('delete')
                    <button type="submit" class="btn btn-outline-danger">Delete</button>
                </form>
            </div>
            <form role="form" action="{{route('novouncher.update',$d->id)}}" method="POST">
                @csrf
                @method('put')
